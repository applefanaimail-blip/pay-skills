...
name: llm
title: "BridgeNode"
description: "Accountless OpenAI-compatible LLM chat completions (DeepSeek, Groq) with per-request x402 payments in Solana USDC — no API keys, no registration."
use_case: "Use for pay-per-call LLM inference through the OpenAI request shape when an agent needs chat completions, model routing, or pricing transparency without API keys or accounts."
category: ai_ml
